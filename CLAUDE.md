...
Seção "📊 Visão estratégica" (`index.html`, logo após o Resumo geral) com 4 visualizações — pedida pelo usuário em 2026-09-04 para dar uma leitura visual rápida do funil de adesão, complementar aos cards de KPI numéricos. Ajustada no mesmo dia (2ª rodada de feedback: "poderia ter visão por município e por ubs e faltou algo mais visual para a fase II"):

- **Toggle Municípios/UBS** (`setModoVisao()`, botões `.toggle-btn` no cabeçalho da seção): alterna a unidade de contagem em todos os 4 gráficos da seção (`modoVisao` global, `'municipios' | 'ubs'`) sem refazer chamada à API — `?action=stats` e `?action=ranking_uf` já trazem os dois totais juntos (campos `*_municipios` e `*_ubs_total`/`mun_*`/`ubs_*`), o toggle só troca qual campo é lido e re-renderiza a partir do último payload guardado (`ultimoStats`/`ultimoRankingUf`).
- **Donut de situação** (`renderDonutPrincipal()`): 3 fatias (não aderiu / Fase II pendente / Fase II concluída) do universo elegível inteiro.
- **Gauge de Fase II** (`renderDonutFase2()`): donut de 2 fatias só entre quem já aderiu (drill-down, não mistura com "não aderiu") — centro mostra o **% concluído da Fase II** em destaque. Adicionado porque o donut principal misturava as 3 situações e ficava pouco visual para acompanhar especificamente o avanço da Fase II.
- **Funil de progresso** (`renderFunil()`): 3 barras horizontais (Elegíveis → Aderiram → Fase II concluída) com largura proporcional ao total.
- **Ranking por UF** (`carregarRankingUf()` / `renderRankingUf()`): barra horizontal empilhada por UF, ordenada pelas UFs com mais municípios não aderidos ou pendentes de Fase II. Consome `?action=ranking_uf` em `sync-fust.php`, que retorna as duas unidades por UF (`mun_nao_aderiu`/`mun_fase2_pendente`/`mun_fase2_completa` e `ubs_nao_aderiu`/`ubs_fase2_pendente`/`ubs_fase2_completa`) — respeita os filtros de UF/município.

O donut e o gauge são desenhados pela mesma função genérica `renderDonutGenerico()` (`<circle>` SVG + `stroke-dasharray`/`stroke-dashoffset`, técnica de anel sem lib de gráficos). Cores reaproveitadas das mesmas `--red`/`--yellow`/`--green` já usadas no farol e no mapa (constante `CORES_STATUS` no JS, valores hex fixos porque `var()` do CSS não é confiável em atributos de presentation SVG como `stroke`) — nenhuma paleta nova foi introduzida. Nenhuma lib de gráficos foi adicionada (SVG/CSS puro), mantendo a exceção à regra de vanilla JS restrita a ExcelJS e Leaflet.

**Atenção**: `?action=ranking_uf` (rota nova) e os campos novos de `?action=stats` (`fase2.completa_ubs_total`, `fase2.completa_ubs_sem_venc`, `fase2.pendente_ubs_total`, `fase2.pendente_ubs_sem_venc`) dependem do `sync-fust.php` atualizado — como esse arquivo é gitignored, precisa ser reenviado manualmente para o Hostinger (ver "Regras principais de desenvolvimento") antes da visão "por UBS" e do ranking por UF funcionarem em produção; até lá, essas partes específicas mostram erro/números zerados, mas o resto do painel continua funcionando normalmente.
