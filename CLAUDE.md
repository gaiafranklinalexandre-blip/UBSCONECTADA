...
- **Linha do tempo** (`renderLinhaTempo()` / `renderLinhaTempoAdesao()`): 2 gráficos de linha (Municípios / UBS) com o acumulado histórico (linha sólida, `<path>` SVG) e uma **linha tracejada de projeção** do último ponto real até a "Meta" (= universo elegível total, `stats.resumo_geral.municipios`/`ubs_total`) — réplica do estilo de uma planilha de controle que o usuário já mantinha manualmente (captura de tela compartilhada em 2026-09-04). `renderLinhaTempo(svgId, pontos, meta, cor)` é genérico (eixo Y de 0 até `max(meta, último valor)`, só 3 rótulos de eixo X para não poluir); `renderLinhaTempoAdesao()` monta os pontos acumulando `ultimoSerieAdesoes` (mesma lista diária do ritmo de adesão) e injeta a meta de `ultimoStats`.
