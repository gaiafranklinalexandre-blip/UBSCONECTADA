...
- **Aplicado no painel real** (`index.html`), no mesmo dia, a pedido do usuário ("aplique o design system no UBS Conectadas para eu validar" → "pode aplicar direto na página real") — ver "Aplicação do design system no index.html" logo abaixo. **Continua vanilla JS/CSS/HTML** (nenhum React foi embutido no painel — só o visual/tokens/markup foram portados manualmente, por regra do projeto).
- **Próximo passo em aberto**: rodar a skill `/design-sync` a partir de `design-system/` para efetivamente publicar em claude.ai/design (shape "package", sem Storybook) — ainda não executado nesta sessão (o usuário pediu para validar o visual no painel real primeiro).

### Aplicação do design system no `index.html` (reestruturação completa, 2026-09-04)

Escopo confirmado com o usuário via pergunta explícita (reestruturação completa vs. só refinar tokens — escolheu completa). Tudo em vanilla HTML/CSS, direto no `<style>` e no corpo do `index.html` — nenhuma dependência nova, nenhum React embutido:

- **`.govbr-bar`**: nova barra no topo do `.header`, acima do título do painel — wordmark "gov.br" tipográfico (sem o arquivo oficial da logo) + "Ministério da Saúde" + ícones utilitários (📊🎧💬◐), espelhando `GovBrHeader` do design system.
- **`.breadcrumb`**: trilho "🏠 › Ministério da Saúde › SAPS › UBS Conectadas" acima do título "UBS Conectadas" dentro de `.header-inner`, espelhando `Breadcrumb`.
- **`.brand-banner`**: banner decorativo (textura diagonal + arco verde SVG + pontilhado vermelho, via `CORES_STATUS`-like tokens `--brand-green`/`--brand-red`/`--link-blue`) logo no início do `#dashBody`, com o texto institucional do programa — espelhando `BrandBanner`.
- **`.secao-titulo`**: todos os títulos de seção (📋 Resumo geral, ✅ Adesão, 📊 Visão estratégica etc.) passaram de alinhados à esquerda para **centralizados com linha horizontal dos dois lados** (via `::before`/`::after` flex, sem alterar o HTML de cada seção) — espelhando `SectionTitle`.
- **`.app-footer`**: rodapé navy (`--navy: #071D41`) no fim da página — "Ministério da Saúde | SAPS © 2026" — espelhando `Footer`. Não existia nenhum rodapé antes.
- KPI cards, pills de status (farol), botões e tabelas **já batiam** com os tokens do design system (foi o próprio `index.html` que originou o núcleo azul/verde/amarelo/vermelho) — não precisaram de ajuste.
- Novos tokens adicionados ao `:root`: `--navy`, `--link-blue`, `--brand-green`, `--brand-red` (mesmos valores amostrados por pixel usados em `design-system/src/styles/tokens.css`).
- **Não visualmente conferido num navegador real** (sem Chromium/`chromium-cli` neste ambiente) — só validado por leitura de HTML/CSS e teste de que a página carrega sem erro num servidor local. Pedir para o usuário conferir ao vivo depois do deploy no Render antes de considerar definitivo.
