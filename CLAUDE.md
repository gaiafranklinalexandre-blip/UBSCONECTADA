...
**Atualização 2026-09-05**: o pacote React antigo do projeto foi arquivado em `_arquivo/design-system-obsoleto-2026-09-05/` e substituído integralmente pelo pacote exportado em `C:\Users\franklin.santos\Downloads\Gerência APS Design System`. O `index.html` deve consumir `design-system/styles.css` e usar os tokens oficiais do novo pacote (Rawline, `--blue-500`, `--color-surface-page`, `--color-border-default`, raios pequenos e bordas 1px). Esta decisão substitui a direção visual "editorial de dados" anterior.
...
- **Histórico superado pela atualização de 2026-09-05**: a orientação anterior dizia para o painel não imitar a identidade institucional do Gerência APS/Ministério da Saúde. O usuário mudou a direção e pediu explicitamente para aplicar o pacote `Gerência APS Design System` ao projeto e publicar o painel online. Continua válida a cautela de não inventar caminhos de navegação, breadcrumbs ou fluxos que não existam.
...
4. **2026-09-07 — de relatório de rolagem para app de sistema**: usuário pediu explicitamente "quero que o painel fique com cara de sistema". Perguntado se isso significava só polir a página atual ou reestruturar como app com abas/sidebar, escolheu a reestruturação. Trocou o `.header` + `.top-menu` (nav por âncora, rolagem única de ~1450 linhas) por um shell de app: `<aside class="app-sidebar">` fixa à esquerda com as 10 telas (`resumo`, `prioridades`, `visao-estrategica`, `evolucao`, `adesao`, `faltam-aderir`, `fase-ii`, `contatos`, `detalhamento`, `mapa`), `.app-topbar` com os filtros de UF/Município sempre visível dentro de `.app-main`, e cada seção antiga virou um `<section class="view-section" id="view-X">` — só uma fica com `display:block` (`.active`) por vez, trocada por `irParaView(id)` no JS (sem refazer chamadas à API: `carregarTudo()` continua carregando tudo de uma vez, só a exibição muda). Deep-link por hash preservado (`#resumo` etc. continuam funcionando, agora via `history.replaceState`/`hashchange` em vez de scroll nativo). Removido também o sublinhado decorativo `::after` dos `.secao-titulo` (cada título agora é o cabeçalho único da sua tela, não um marcador em meio a um scroll longo) e a animação `kpiEntrada`/`animation-delay` dos KPI cards, que já estava morta no CSS (nunca era de fato aplicada via `animation:`) — resquício da direção "editorial" do item 3, acima, que já não fazia sentido numa tela estática de sistema.
   - **Cuidado ao reabrir o mapa**: como a tela "Mapa de adesão" começa escondida (`display:none`), o Leaflet (`initMapa()`, chamado de cara no boot) inicializa com o `#mapaAdesao` em 0×0. `irParaView('mapa')` chama `mapaLeaflet.invalidateSize()` + refit dos últimos pontos (`ultimoMapaPontos`, preenchido em `carregarMapa()`) num `setTimeout(...,0)` depois de aplicar `.active` — sem isso o mapa fica com tiles cortados/zoom errado na primeira visita à tela.
- **Não visualmente conferido num navegador real** em nenhuma das etapas, incluindo a reestruturação de 2026-09-07 (sem Chromium/`chromium-cli` neste ambiente) — só validado por: leitura de HTML/CSS, `node --check` no JS extraído do `<script>` inline (sintaticamente válido), teste de que a página carrega sem erro 200 num servidor HTTP local, e contagem de `<section>`/`</section>` batendo (10/10). Pedir para o usuário conferir ao vivo no Render antes de considerar definitivo — em especial o comportamento do Leaflet ao trocar para a tela do mapa e o layout responsivo da sidebar em telas estreitas (vira barra horizontal de abas abaixo de 900px).
